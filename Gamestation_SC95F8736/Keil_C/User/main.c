...
extern char minute;
extern char second;
extern char set_hour;
extern char set_minute;
extern char set_second;
extern char set_numbuf[6];
extern char clock_mode;
u16 CLK_POINT_COLOR=0x001f;
u16 CLK_POINT_COLOR_DEF=0x0000;
void judge_clock(void)
{
	if(set_hour==hour && set_minute==minute && set_second==second)
	{
		Lcd_Clear(0xFFFF);
		Gui_DrawFont_GBK24(20,50,GRAY1,BACK_COLOR,"TIME OUT!!!");
		Delay_ms(5000);
                    ...
                    while(Start==1)
                    {
						...
						if(clock_mode==0)
						{
							...
							else 
							{
								...
								sprintf(set_numbuf,"%d",(int)set_second);
								Gui_DrawFont_GBK24(58,20,POINT_COLOR,BACK_COLOR,set_numbuf);
							}
							sprintf(set_numbuf,"%d:",(int)set_hour);
							Gui_DrawFont_GBK24(0,20,CLK_POINT_COLOR,BACK_COLOR,set_numbuf);
						...
						if(clock_mode==1)
						{
							...
							sprintf(set_numbuf,"%d:",(int)set_minute);
							Gui_DrawFont_GBK24(25,20,CLK_POINT_COLOR,BACK_COLOR,set_numbuf);
						}
						if(clock_mode==2)
						{
							if(set_second>=10)
							{
								sprintf(set_numbuf,"%d",(int)set_second);
								Gui_DrawFont_GBK24(50,20,CLK_POINT_COLOR,BACK_COLOR,set_numbuf);
							}
							else 
							{
								Gui_DrawFont_GBK24(50,20,CLK_POINT_COLOR,BACK_COLOR,"0");
								sprintf(set_numbuf,"%d",(int)set_second);
								Gui_DrawFont_GBK24(58,20,CLK_POINT_COLOR,BACK_COLOR,set_numbuf);
							}
							sprintf(set_numbuf,"%d:",(int)set_hour);
							Gui_DrawFont_GBK24(0,20,POINT_COLOR,BACK_COLOR,set_numbuf);
							sprintf(set_numbuf,"%d:",(int)set_minute);
...
void run_games2(void)
{
    while(rev_key!=KEY_Start)	judge_clock();;
    init_interface2_info();
    while(1)
    {
		judge_clock();
