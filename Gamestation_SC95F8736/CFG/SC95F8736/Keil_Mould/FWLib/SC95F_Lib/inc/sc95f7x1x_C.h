...
/*PSW*/
sbit  CY	  = PSW ^
              7;	//标志位		0:加法运算最高位无进位，或者减法运算最高位无借位时		1:加法运算最高位有进位，或者减法运算最高位有借位时
sbit  AC	  = PSW ^
              6;	//进位辅助标志位	0:无借位、进位		1:加法运算时在bit3位有进位，或减法运算在bit3位有借位时
sbit  F0	  = PSW ^ 5;	//用户标志位
...
/*IP*/
sbit  IPADC	  = IP ^
                6;	//ADC中断优先控制位      0:设定 ADC的中断优先权是 “低”		1:设定 ADC的中断优先权是 “高”
sbit  IPT2	  = IP ^
                5;	//Timer2中断优先控制位   0:设定 Timer2的中断优先权是 “低”		1:设定Timer2的中断优先权是 “高”
sbit  IPUART  = IP ^
                4;	//UART中断优先控制位	   0:设定UART的中断优先权是 “低”		1:设定UART的中断优先权是 “高”
sbit  IPT1	  = IP ^
                3;	//Timer1中断优先控制位   0:设定 Timer 1的中断优先权是 “低”	1:设定 Timer 1的中断优先权是 “高”
sbit  IPINT1  = IP ^
                2;	//INT1中断优先控制位	   0:设定INT1中断优先权是 “低”		    1:设定INT1的中断优先权是 “高”
sbit  IPT0	  = IP ^
                1;	//Timer0中断优先控制位   0:设定 Timer 0的中断优先权是 “低”	1:设定 Timer 0的中断优先权是 “高”
sbit  IPINT0  = IP ^
                0; //INT0中断优先控制位	   0:设定INT0中断优先权是 “低”		    1:设定INT0的中断优先权是 “高”
...
/*TCON*/
sbit  TF1	  = TCON ^
              7;	//T1溢出中断请求标志位	T1产生溢出，发生中断时，硬件将TF1置为“1”，申请中断，CPU响应时，硬件清“0”。
sbit  TR1	  = TCON ^ 6;	//定时器T1的运行控制位	0:Timer1禁止计数		1:Timer1开始计数
sbit  TF0	  = TCON ^
              5;	//T0溢出中断请求标志位	T0产生溢出，发生中断时，硬件将TF0置为“1”，申请中断，CPU响应时，硬件清“0”。
...
*如若选SC95F7517，则不用调用宏定义。
*如若选SC95F7617，则不用调用宏定义。
*****************************************************************************/                                    
#define  SC95F7516_NIO_Init()   {P4CON|=0xC0,P5CON|=0x30;}                                                 //SC95F7516的未引出来的IO口配置
#define  SC95F7515_NIO_Init()   {P0CON|=0x0F,P1CON|=0xF0,P3CON|=0xF0,P4CON|=0xC0,P5CON|=0x30;}             //SC95F7515的未引出来的IO口配置
#define  SC95F7513_NIO_Init()   {P0CON|=0x0F,P1CON|=0xF0,P3CON|=0xF0,P4CON|=0xC0,P5CON|=0x3F;}             //SC95F7513的未引出来的IO口配置
#define  SC95F7512_NIO_Init()   {P0CON|=0x0F,P1CON|=0xF0,P2CON|=0xF0,P3CON|=0xFF,P4CON|=0xC0,P5CON|=0x3F;} //SC95F7512的未引出来的IO口配置

#define  SC95F7619_NIO_Init()   {P0CON|=0x01,P3CON|=0xFF,P4CON|=0x01,P5CON|=0x3F;}                         //SC95F7619的未引出来的IO口配置
#define  SC95F7616_NIO_Init()   {P4CON|=0xC0,P5CON|=0x30;}                                                 //SC95F7616的未引出来的IO口配置
#define  SC95F7615_NIO_Init()   {P0CON|=0x0F,P1CON|=0xF0,P3CON|=0xF0,P4CON|=0xC0,P5CON|=0x30;}             //SC95F7615的未引出来的IO口配置
#define  SC95F7613_NIO_Init()   {P0CON|=0x0F,P1CON|=0xF0,P3CON|=0xF0,P4CON|=0xC0,P5CON|=0x3F;}             //SC95F7613的未引出来的IO口配置
#define  SC95F7612_NIO_Init()   {P0CON|=0x0F,P1CON|=0xF0,P2CON|=0xF0,P3CON|=0xFF,P4CON|=0xC0,P5CON|=0x3F;} //SC95F7612的未引出来的IO口配置
